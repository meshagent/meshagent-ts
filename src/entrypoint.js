...
var __commonJS = (cb, mod) => function __self._require() {
...
var ServerXmlDocument = class {
   ...
   */
  constructor(doc2, undo, notifyChanges) {
    this._y = doc2.get("xml", YXmlElement);
    if (undo) {
      this._undoManager = new UndoManager(this._y);
    } else {
    ...
    this._y.observeDeep((yxmlEvents) => {
      for (const yxmlEvent of yxmlEvents) {
        ...
        if (target instanceof YXmlElement) {
          ...
          for (const delta of yxmlEvent.changes.delta) {
            if (delta.insert) {
              message.elements.push({
                retain: delta.retain,
                insert: (delta.insert instanceof Array ? delta.insert : [delta.insert]).map((i) => this.serialize(i))
              });
          ...
          const nodeID = parent?.getAttribute("$id") ?? void 0;
          const message = {
            root: false,
            target: nodeID,
            elements: [],
   ...
   */
  doDelete(element2) {
    if (element2.parent instanceof YXmlElement) {
      let i = 0;
      let cur = element2.parent.firstChild;
      while (cur) {
        if (cur === element2) {
  ...
  createNodes(children) {
    ...
    for (const child of children) {
      if (child.text) {
        ...
        if (child.text.delta) {
          text2.applyDelta(child.text.delta);
        }
        nodes.push(text2);
      } else if (child.element) {
        const element2 = new YXmlElement(child.element.name);
        element2.setAttribute("$id", child.element.attributes?.id ?? v4_default());
  ...
  applyChanges(changes) {
    for (const change of changes) {
      ...
      if (change.insertChildren) {
        if (!(elementOrText instanceof YXmlElement)) {
...
function getState2(documentID, vector) {
  ...
  let state;
  if (vector) {
    const decodedVector = import_base_64.default.decode(vector);
    const stateVector = Uint8Array.from(decodedVector, (c) => c.charCodeAt(0));
    state = encodeStateAsUpdate(server.doc, stateVector);
  } else {
...
function registerDocument(id2, base64Data, undo = false, sendUpdateToBackend, sendUpdateToClient) {
  ...
  const server = new ServerXmlDocument(clientProtocol.doc, undo, (update) => {
    const msg = JSON.stringify({ documentID: id2, data: update });
  ...
  if (base64Data) {
    const buffer = Uint8Array.from(import_base_64.default.decode(base64Data), (c) => c.charCodeAt(0));
    server.applyBackendChanges(buffer);
  }
