...
import { JsonContent, FileContent } from "./response";
...
export class StorageClient extends EventEmitter<RoomEvent> {
  ...
  public async list(path: string): Promise<StorageEntry[]> {
    const response = (await this.client.sendRequest("storage.list", { path })) as JsonContent;
  ...
  public async open(path: string, {overwrite = false}: {overwrite: boolean}): Promise<FileHandle> {
    const response = (await this.client.sendRequest("storage.open", { path, overwrite })) as JsonContent;
  ...
  public async exists(path: string): Promise<boolean> {
    const result = (await this.client.sendRequest("storage.exists", { path })) as JsonContent;
  ...
  public async download(path: string): Promise<FileContent> {
    const response = (await this.client.sendRequest("storage.download", { path })) as FileContent;
  ...
  public async downloadUrl(path: string): Promise<string> {
    const response = (await this.client.sendRequest("storage.download_url", { path })) as JsonContent;
